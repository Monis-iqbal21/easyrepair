...
export class ChatService {
  ...
  async sendMessage(
    ...
    const dto = this._toMessageDto(message);
  ...
  async sendMediaMessage(
    ...
    if (isVideo && thumbnailBuffer && thumbnailOriginalName) {
      try {
        ...
        this.logger.warn(
          `[ChatService] thumbnail upload failed: ${(err as Error)?.message}`,
        );
      }
    }
