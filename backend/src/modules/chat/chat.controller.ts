...
export class ChatController {
   ...
   * Fields: "file" (required, image/* or video/*, max 50 MB),
   *         "thumbnail" (optional, image/jpeg, attached by client for video messages).
   */
    ...
    FileFieldsInterceptor(
      ...
      ],
      {
        fileFilter: (_req, file, cb) => {
          const isMainFile = file.fieldname === 'file';
          const allowed = isMainFile
