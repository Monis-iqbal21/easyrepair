...
import 'package:handygo_app/features/worker/presentation/pages/worker_suspended_page.dart';
...
void main() {
  ...
  Future<_FakeLogoutNotifier> pump(WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: localizedApp(const WorkerSuspendedPage()),
      ),
    );
  ...
  testWidgets('shows the suspended-account message', (tester) async {
    await pump(tester);
    expect(
      find.text(
  ...
  testWidgets('renders no bottom navigation — nothing to tap into', (tester) async {
    await pump(tester);
    expect(find.byType(WorkerBottomNavBar), findsNothing);
  });
