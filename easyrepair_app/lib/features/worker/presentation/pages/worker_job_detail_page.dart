...
/// Calls the Google Directions API (driving mode) and returns decoded route
/// points, or null when the key is missing or the request fails.
///
/// TODO: Enable "Directions API" in Google Cloud Console for GOOGLE_MAPS_API_KEY,
...
class _LocationSectionState extends ConsumerState<_LocationSection>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      // Cancel the distance-check timer while backgrounded — GPS calls in the
      // background are restricted and the timer would run needlessly.
      // Keep _directionsActive = true so it resumes when the app returns.
      _dirTimer?.cancel();
      _dirTimer = null;
      debugPrint('[DirectionsMode] App paused — timer suspended.');
    } else if (state == AppLifecycleState.resumed) {
      // Restart the timer if directions were still active when we paused.
      if (_directionsActive) {
        debugPrint('[DirectionsMode] App resumed — restarting timer.');
        _startDirTimer();
      }
      // Refresh job detail in case status changed while backgrounded.
  ...
  Future<void> _startDirections() async {
    ...
    // No Directions API key — skip in-app route, open external navigation.
    ...
    // Prefer the tracker's last synced location (already proven fresh).
    ...
    // Activate markers with initial two-point bounds while route loads.
    setState(() {
    ...
    if (route != null && route.isNotEmpty) {
      ...
      // Emergency straight-line fallback — Directions API returned no route.
      // TODO: Verify GOOGLE_MAPS_API_KEY has "Directions API" enabled in
  ...
  Future<void> _openExternalMaps() async {
    // Stop the internal directions session before switching to external nav —
    // the distance timer should not run while the app is not in the foreground.
  ...
  Set<Polyline> _buildPolylines() {
    ...
    // Road route from Directions API — primary path.
    ...
    // Emergency straight-line fallback: shown only when the Directions API
    // returned no route (network error, quota, or missing API enablement).
    // TODO: Verify GOOGLE_MAPS_API_KEY has "Directions API" enabled in
    //       Google Cloud Console.
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: GestureDetector(
                    onTap: () async {
                      // Suspend preview timer while fullscreen is open to avoid
                      // two timers polling GPS simultaneously.
                      _dirTimer?.cancel();
                      _dirTimer = null;
                      final result =
                      ...
                      if (!mounted) return;
                      // Sync directions state returned from fullscreen.
                      if (result != null) {
                        setState(() {
                      ...
                      // Restart preview timer if directions are still active.
                      if (_directionsActive) {
...
class _FullScreenMapPageState extends ConsumerState<_FullScreenMapPage>
  ...
  void initState() {
    ...
    // Start distance timer immediately if directions were already active.
  ...
  /// Always pop with the current directions state so the preview can sync.
  void _popWithResult() {
    Navigator.pop(
      context,
  ...
  // ── Directions (mirrors _LocationSectionState logic) ────────────────────────
        ...
        body: Stack(
          children: [
            GoogleMap(
              ...
              onMapCreated: (c) {
                _ctrl = c;
                // Fit bounds if directions were passed in from the preview.
                if (_directionsActive) {
