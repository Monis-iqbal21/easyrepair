...
class _ChatDetailPageState extends ConsumerState<ChatDetailPage> {
  ...
  Future<void> _sendVoiceFile(String path) async {
    ...
    debugPrint('[ChatVoice] file path = $path');
    debugPrint('[ChatVoice] file exists = $exists');
    if (!exists) {
      debugPrint('[ChatVoice] file not found — aborting upload');
      _showError('Voice recording file not found.');
      return;
    }
    try {
      debugPrint('[ChatVoice] file size = ${await voiceFile.length()}');
    } catch (_) {}
    setState(() => _isSendingAttachment = true);
    debugPrint('[ChatVoice] upload started');
    try {
      final result = await ref
          .read(chatRepositoryProvider)
      ...
      result.fold(
        (failure) {
          debugPrint('[ChatVoice] upload failed: ${failure.toString()}');
          _showError(failure.toString());
        },
        (message) {
          debugPrint('[ChatVoice] upload success');
          ref
              .read(chatMessagesProvider(widget.conversationId).notifier)
              .append(message);
          WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
      ...
      debugPrint('[ChatVoice] upload exception: $e');
        ...
        children: [
          ...
          if (_isRecording)
            _VoiceRecordBar(
              ...
              onPause: _pauseVoiceRecording,
              onResume: _resumeVoiceRecording,
            )
          else
            _InputBar(
    ...
    return GestureDetector(
      ...
      child: ConstrainedBox(
        ...
        child: Column(
          ...
          mainAxisSize: MainAxisSize.min,
          children: [
            // Static map thumbnail when API key is configured
            if (hasCoords && apiKey.isNotEmpty) ...[
              ClipRRect(
...
class _VoiceRecordBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Status dot: red while recording, grey while paused
          Container(
            width: 10,
            height: 10,
            decoration: BoxDecoration(
          ...
          const SizedBox(width: 10),
          Text(
            '${isPaused ? 'Paused' : 'Recording'}  ${_fmt(duration)}',
            style: const TextStyle(
              fontSize: 14,
              color: Color(0xFF1A1A1A),
              fontWeight: FontWeight.w500,
