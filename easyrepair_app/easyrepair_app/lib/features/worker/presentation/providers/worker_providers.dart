...
class LocationTrackerNotifier extends Notifier<LocationTrackingState> {
  ...
  Future<AvailabilityStatus> startTracking() async {
    ...
    if (position != null) {
      ...
      // Block going online when location is unavailable — do not send stale/null coords.
      final perm = await Geolocator.checkPermission();
      final msg = perm == LocationPermission.deniedForever
          ? 'Location permission permanently denied. Enable it in Settings to go online.'
      ...
      debugPrint('[LocationTracker] Blocking go-online: $msg');
      throw ServerFailure(msg);
    }

    // Immediate server update with confirmed current location
